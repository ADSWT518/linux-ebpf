...
#include "illegal_kern.skel.h" // 由 bpftool 生成的头文件
...
int main(int argc, char **argv) {
    struct illegal_kern *skel;
    int err = 0;

    // 1. 打开、加载并验证 BPF 程序骨架
    skel = illegal_kern__open_and_load();
    if (!skel) {
